...
(defun eval* (expression environment)
  "Classifies an expression and directs its evaluation."
  (cond ((self-evaluatingp expression)
	 expression)
	((variablep expression)
	...
	((lambdap expression)
	 (make-procedure (lambda-parameters expression)
			 ...
			 environment))
	((prognp expression)
	 (eval-sequence (progn-actions expression) environment))
	((condp expression)
	 (eval* (cond->if expression) environment))
	((applicationp expression)
...
(defun apply* (procedure arguments)
  "Classifies procedures into primitive and compound procedures and applies the relevant evaluation."
...
(defun list-of-values (expressions environment)
  ...
  (if (no-operandsp expression)
      nil
      (cons (eval* (first-oeprand expressions) environment)
	    (list-of-values (rest-operands expressions) environment))))


(defun eval-if (expression environment)
  "Evaluates predicate part of an if expression in the given environment. If the result is true, it evaluates the consequence, otherwise it evaluates the alternative."
  (if (truep (eval* (if-predicate expression) environment))
      (eval* (if-consequent expression) environment)
      (eval* (if-alternative expression) environment)))


(defun eval-sequence (expressions environment)
  "Evaluates expressions in the order in which they occur and returns the value of the final expression."
  (cond ((last-expressionp expressions)
	 (eval* (first-expression expressions) environment))
	(t
	 (eval* (first-expression expressions) environment)
	 (eval-sequence (rest-expressions expressions) environment))))


(defun eval-assignment (expression environment)
  "Finds value to be assigned and installs in the designated environment."
...
;; We have chosen here to return the symbol ok as the value of
;; an assignment or a definition (but this is up to you)

(defun eval-definition (expression environment)
...
(defun self-evaluatingp (expression)
  (cond ((numberp expression) t)
	((stringp expression) t)
	(t nil)))

(defun variablep (expression)
  (symbolp expression))

(defun quotedp (expression)
  (tagged-listp expression 'quote))

(defun text-of-quotation (expression)
  (cadr expression))

(defun tagged-listp (expression tag)
  (if (pairp expression)
      (eq (car expression) tag)
      nil))

;; Need to define pairp

(defun assignmentp (expression)
  (tagged-listp expression 'set))

(defun assignment-variable (expression)
  (cadr expression))

(defun assignment-value (expression)
  (caddr expression))

(defun definition (expression)
  (or (tagged-listp expression 'defvar)
      (tagged-listp expression 'defun)))

;; Note the forms for defvar & defun:
;; (defvar <var> <value>)
;; (defun <var> <parameters> <body>)

(defun definition-variable (expression)
  (cadr expression))

(defun definition-value (Expression)
  (if (symbolp (caddr expression))
      (caddr expression)
      (make-lambda (caddr expression)
		   (cdddr expression))))

(defun lambdap (expression)
  (tagged-listp expression 'lambda))

(defun make-lambda (parameters body)
  (cons 'lambda (cons parameters body)))

(defun ifp (expression)
  (tagged-listp expression 'if))

(defun if-predicate (expression)
    (cadr expression))

(defun if-consequent (expression)
    (caddr expression))

(defun if-alternative (expression)
  (if (not (null (cdddr expression)))
      (cadddr expression)
      nil))

(defun prognp (expression)
  (tagged-listp expression 'progn))

(defun progn-actions (expression)
  (cdr expressions))

(defun last-expression-p (sequence)
  (null (cdr sequence)))

(defun first-expression (sequence)
  (car sequence))

(defun rest-expressions (sequence)
  (cdr sequence))

(defun sequence->expression (sequence)
  (cond ((null sequence) sequence)
	((last-expresion-p sequence)
	 (first-expression sequence))
	(t (make-progn sequence))))

(defun make-progn (sequence)
  (cons 'progn sequence))

(defun application-p (expression)
  (pairp expression))

(defun operator (expression)
  (car expression))

(defun operands (expresion)
  (cdr expression))

(defun no-operands-p (operations)
  (null operations))

(defun first-operand (operations)
  (car operations))

(defun rest-operands (operations)
  (cdr operations))


;; Derived Expressions

(defun condp (expression)
  (tagged-listp expression 'cond))

(defun cond-clauses (expression)
  (cdr expression))

(defun cond-else-clause-p (clause)
  (eq (cond-predicate clause) t))
...
(defun make-procedure (parameters body environment)
  (list 'procedure parameters body environment))

(defun compound-procedure-p (procedure)
  (tagged-list-p procedure 'procedure))
...
(defun enclosing-environment (environment)
  (cdr environment))

(defun first-frame (environment)
  (car environment))
...
(defun extend-environment (variables values base-environment)
  (if (= (length variables) (length values))
      (cons (make-frame variables values) base-environment)
      (if (< (length variables) (length values))
	  (error "Too many arguments supplied" variables values)
	  (error "Too few arguments supplied" variabels values))))

(defun lookup-variable-value (var environment)
  (labels ((environment-loop (environment)
	     (labels ((scan (variables values)
			(cond ((null variables)
			       (environment-loop (enclosing-environment environment)))
			      ((equal var (car variables))
			       (car values))
			      (t (scan (cdr variables)
				       (cd values))))))
...
(defun set-variable-value (var val environment)
  (labels ((environment-loop (environment)
	     (labels ((scan (variables values)
			(cond ((null variables)
			       (environment-loop (enclosing-environment environment)))
...
(defun define-variable (var val environment)
  (let ((frame (first-frame environment)))
    (labels ((scan (variables values)
	       (cond ((null variables)
